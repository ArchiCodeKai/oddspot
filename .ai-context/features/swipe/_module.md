...
src/components/swipe/
  ...
  SwipeDecisionAnimation.tsx ← skip / save 的 Lottie decision feedback（edge hint + center stamp）
  SwipeActionBar.tsx      ← 浮在卡片底部上方的 skip / save / save + trip 圓形按鈕（tooltip + press-only）
...
| 右滑 / 打勾 | 加進收藏夾 + 今日探險行程 | `useSavedStore.addSave` + `useRoutePlannerStore.addSpot` |
| 中間 + 按鈕 | 只加進收藏夾 | `useSavedStore.addSave(id)` |
...
- 卡片內的 `導航前往 Google Maps` 是單點外部導航，不加入路線排程；使用者要排程可右滑或按打勾，系統會同時收藏並加入今日行程
...
SwipeCard 堆疊（顯示前 3 張）
  ...
  ├─ 右滑 / 打勾 → useSavedStore.addSave + useRoutePlannerStore.addSpot
  ├─ 中間 + 按鈕 → useSavedStore.addSave
...
- action button 採圓形 64px touch target；中間收藏按鈕 76px，使用寬胖圓角十字作為視覺主按鈕；打勾才會收藏並加入今日行程
- action button 已移除內側同心圓；hover / focus 只顯示文字 tooltip；active 只做按壓，不讓按鈕本身承擔結果動畫
- skip / save 動畫改由 `SwipeDecisionAnimation` 播放 `public/lottie/swipe/cross.json` / `check.json`；Lottie 只替換內層圖形動態，action button 與中央 acid stamp 外框都沿用現有設計
- `SwipeDecisionAnimation` 會讀 `--accent-rgb` / `--muted-rgb` 後用 `src/lib/lottie/tintLottie.ts` runtime tint JSON shape color，讓 theme 切換後的 feedback 顏色跟著主題走
- 為避免第一次互動時資源還沒載完，`SwipeCard` mount 後會呼叫 `preloadSwipeDecisionAnimations()` 預載 skip / save JSON；center stamp 才播放 Lottie。edge hint 保持 SVG，避免拖曳時渲染大型 Lottie 掉幀；`prefers-reduced-motion` 則回到靜態 SVG fallback
- 卡片 flyOut 完成後延遲 620ms 才 advance 到下一張，讓中央 decision feedback 不會播到一半被卸載
- 手機版首屏不顯示 action button，需滾到卡片底部才看到 68/82px 觸控按鈕；桌機版保留卡片外浮動按鈕。三顆按鈕邏輯為：叉叉跳過、加號只收藏、打勾收藏並加入今日行程
...
- SwipeCard 的觸控手勢由 `.acid-card-scroll` 接管 capture 事件；水平拖曳開始後會暫時將該 scroll 容器 `overflowY` 改成 `hidden`，pointer end / cancel 後恢復 `auto`，避免行動裝置 swipe 到一半像撞到 scrollbar 一樣卡住
- 手機與桌機拖曳時，左右邊緣會出現低透明度 X / check icon 往中心滑入並逐漸變淡，切換下一張後消失；桌機仍保留文字 stamp
- skip / save 結果回饋在卡片中心顯示 1.1s stamp；加入路線顯示資料夾 icon，卡片先回到中心再縮小淡出，形成收進資料夾的 intake 動畫
