...
2. 列出本次修改的所有檔案清單
3. 更新對應的文件（AGENTS.md 或 .ai-context/ 下的相關 md）
...
### 1. Codex Design（雲端）— 設計原件 Source of Truth
- 專案：Codex.ai/design「OddSpot Design System」
- 匯出方式：Share → Download project as .zip
- 解壓位置：`src/design-reference/Codex-design-source/`
- 不可直接修改此資料夾內容（是 baseline）

### 2. Huashu Design skill — 設計迭代與變體
- 讀取：`src/design-reference/Codex-design-source/` 作為基底
